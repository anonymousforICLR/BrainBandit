...
class UCB_Model:
    def __init__(self, n_choice, tau, mean = 0, mean_std=1, std_list=None, prior_std = None):
        ...
        self.expected_mean = np.zeros(n_choice,dtype=float)
        self.expected_var = np.ones(n_choice, dtype=float) * mean_std * mean_std if prior_std is None else np.array(prior_std)
    ...
    def play(self, total_epochs, c=1., block_id=None, optimal_choice=None):
        for i in range(total_epochs):
            expected_mean = self.expected_mean.copy()
            expected_var = self.expected_var.copy()
            U = expected_mean + c * np.sqrt(expected_var)
...
class Play_UCB:
    def __init__(self, subject: int = 100, block: int = 10, trial: int = 10, bonus:float = 1.,save: bool = True, save_path: str = None,
    ...
    def play(self):
        for subject in range(self.subject):
            ...
            for block in tqdm(range(self.block)):
                model = UCB_Model(n_choice=self.dim, tau=self.tau_list, mean=self.mean, mean_std=self.mean_std, std_list=self.std_list,prior_std = self.prior_std)
                mean_list = model.get_mean_list()
                optimal = np.argmax(mean_list)
                model.play(self.trial, c=self.bonus, block_id=block,optimal_choice = optimal)
...
if __name__ == '__main__':
    ...
    prior_std = [1., 1.]
    tau_list = [1., 1.]
    game1 = Play_UCB(subject=1, block=10000, trial=dim*10, dim=dim,mean_std=1, bonus=bonus,save=True,save_path=f"saved/UCB_2D_{bonus}_{std_list}.csv", save_mode='w',std_list=std_list, tau_list=tau_list, prior_std=prior_std)
    game1.play()
