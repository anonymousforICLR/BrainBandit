...
class Model_epsilon_Greedy:
    ...
    def play(self, total_epochs, p=0., block_id=None, optimal_choice=None):
        for i in range(total_epochs):
            ...
            if sto < p:
                choice = np.random.choice(np.arange(0,self.n,1), size=1)[0]
...
class Play_epsilon_Greedy:
    def __init__(self, subject: int = 100, block: int = 10, trial: int = 10, save: bool = True, save_path: str = None,
                 save_mode: str = 'w',dim = 2, mean = 0, mean_std = 1, epsilon = 0.1, std_list=None,tau_list=None,prior_std = None):
    ...
    def play(self):
        for subject in range(self.subject):
            ...
            for block in tqdm(range(self.block)):
                model = Model_epsilon_Greedy(n_choice=self.dim, tau=self.tau_list, mean=self.mean, mean_std=self.mean_std, std_list=self.std_list, prior_std=self.prior_std)
                mean_list = model.get_mean_list()
                optimal = np.argmax(mean_list)
                model.play(self.trial, p=self.epsilon, block_id=block,optimal_choice = optimal)
...
if __name__ == '__main__':
    ...
    prior_std = [1., 1., 1.]
    tau_list = [1., 1., 1.]
    game1 = Play_epsilon_Greedy(subject=1, block=10000, trial=dim*10, dim=dim,save=True,epsilon=0.1,save_path=f"saved/greedy_{dim}D_{std_list}.csv", save_mode='w',std_list=std_list, tau_list=tau_list, prior_std=prior_std)
    game1.play()
